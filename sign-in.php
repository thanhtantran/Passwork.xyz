<?php
	...
	if (isset($_POST['submit']) && $_POST['submit'] == 'saveSettings') {
			...
			// $stmt = $mysqli->prepare("UPDATE sitesettings SET localization = ?");
			// $stmt->bind_param('sssssss', $localization);
			// $stmt->execute();
			// $stmt->close();
			$sql = "UPDATE sitesettings SET localization = '".$localization."'";
			$res = mysqli_query($mysqli, $sql) or die('-1' . mysqli_error());
										...
										<div class="col-md-6">
											...
											?>
												<select class="form-control" name="localization" id="localization">
													...
													<option value="custom" <?php echo $custom; ?>>Vietnamese</option>
												</select>
											</div>
										</div>
										<div class="col-md-6">
			...
			<div class="container">
				<div class="row">
					<div class="col-md-6 col-md-offset-3">
						<div class="loginCont">
							<div class='login text-center'>
									Dễ dàng để bắt đầu và sử dụng, Passwork là một ứng dụng đơn giản , dễ sử dụng , dựa trên nền web để quản lý mật khẩu bằng văn bản. Những ngày này bạn cần phải nhớ cách để nhiều mật khẩu . Passwork cung cấp một hệ thống dựa trên web với một tài khoản / mật khẩu để nhớ để truy cập vào tất cả các tài khoản của bạn / mật khẩu.

									Bạn chỉ việc thêm tất cả các mật khẩu của bạn vào một tài khoản người dùng duy nhất, tài khoản đó bị khóa với một mật khẩu chủ . Vì vậy, bạn chỉ cần nhớ một mật khẩu duy nhất để mở khóa tất cả các mục mật khẩu của bạn .

									Passwork được thiết kế để có thể sử dụng trên nền tảng mobile bạn có thể truy cập dễ dàng mọi lúc mọi nơi.
							</div>
						</div>
					</div>
				</div>
